...
For spinning up other suspended disks in a zpool, the service executes an
asynchronous 'sdparm' command for each companion disk in the pool. The resume
operation caused by a 'sdparm' process does of course also trigger the service
and without the capability to identify such self-inflicted events it would again
launch a 'sdparm' process for each of the other disks in a zpool. Thus, the
wake-up of a disk in a sleeping zpool with N disks would trigger the execution
of N*(N-1) 'sdparm' processes. Due to its quadratic nature, this could quick
become an unacceptable large number. To avoid this unnecessary proliferation of
'sdparm' processes, the service has to do some bookkeeping.
...
via a 'sdparm' process. The eBPF program on the other side does check the map
for an entry of the device that caused a resume event and only triggers the
Python process if the device is not contained in the map. In case it finds the
device in the map, this means that a resume event caused by the Python process
was pending and therefore just deletes this entry from the map.
                    ...
                    try:
                        if disk.rpm_status[0] == 's':
                            ...
                            try:
                                ...
                                _log_zprpmmgr.error(
                                    "Failed to execute 'sdparm' "
                                    "command for device %s.", disk.blk_dev
                                )
                                _log_zprpmmgr.error(
...
if __name__ == "__main__":
    ...
    if not os.access(CONFIG_FILE, os.R_OK):
        ...
        sys.exit(1)
    # Check that the 'sdparm' utility is available for execution.
    if not os.access(SDPARM, os.X_OK):
        logger.critical(
            "Cannot execute required 'sdparm' utility "
            "under path %s. Terminating Service.", SDPARM
        )
        print(
            "Cannot execute required 'sdparm' utility under path {0}. "
            "Terminating Service.".format(SDPARM), file=sys.stderr
        )
        notify_systemd("STOPPING=1")
        sys.exit(1)
